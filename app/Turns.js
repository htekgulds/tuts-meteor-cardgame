...
Turns.findMatches = function (card, set) {
    ...
    // If there are single card matches, then return them. No need to look further...
...
Turns.bestMatch = function (matches) {
    ...
    var mostCards = [0, null];

    for(var i = 0; i < matches.length; i++) {
        var match = matches[i];

        // 7 of Coins is the most valuable card so if we have it just return it.
        for(var j = 0; j < match.length; j++) {
            if(match[j].suit === 'Coins' && match[j].value === 7) return match;
        }

        // Next most valuable is coins so we count the coins
...
Turns.takeMatch = function (game, id, card, match) {
    ...
    // At the end of the match there may be a few unmatched cards.
    // We give those cards to whoever scored last. So we need to keep track of that
    game.lastScorer = id;
